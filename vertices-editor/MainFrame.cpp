...
wxBEGIN_EVENT_TABLE(MyFrame, wxFrame)
    ...
    EVT_MENU(MyFrame::ID_CreateGL, MyFrame::OnCreateRenderer)
        ...
        : wxFrame(parent, id, title, pos, size, style)
{
    ...
    view_menu->Append(ID_CreateGL, _("Create Renderer Control"));
...
void MyFrame::OnCreateRenderer(wxCommandEvent& WXUNUSED(event))
{
   ...
   auto glPane = new BasicGLPane(this, args);
    ...
    //auto render = new TestGLCanvas(this, NULL);
    
    m_mgr.AddPane(glPane, wxAuiPaneInfo().
                  Caption(wxT("Renderer Control")).
                  Float().FloatingPosition(GetStartPosition()));
    m_mgr.Update();
     * */
}
...
wxGLCanvas* MyFrame::CreateRenderCtrl(wxWindow* parent)
{
    if (!parent)
        parent = this;
...
wxAuiNotebook* MyFrame::CreateNotebook()
{
   ...
   ctrl->AddPage( CreateRenderCtrl(ctrl) , wxT("Render Ctrl 1"), false, page_bmp  );
